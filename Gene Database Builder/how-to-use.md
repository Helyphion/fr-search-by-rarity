...
1. Open `view-source:www1.flightrising.com/scrying/predict` and ctrl + F for `name="bodygene"`, `name="winggene"` or `name="tertgene"`, depending on what has been updated. **If a new breed has been released, remember to manually add it to [`breed-rarities.json`](https://github.com/Helyphion/fr-search-by-rarity/blob/main/breed-rarities.json).**
